...
class MCPClient:
    ...
    def _recv(self, timeout: float = 30.0) -> dict:
        ...
        Uses select() to enforce a read timeout — prevents the agent from
        hanging indefinitely if the MCP server becomes unresponsive.
        """
        assert self.proc and self.proc.stdout
        ready, _, _ = select.select([self.proc.stdout], [], [], timeout)
        if not ready:
            raise TimeoutError(
                f"MCP server 超时（{timeout}s 无响应）。"
                f"服务器可能已挂起或崩溃。请检查 MCP server 进程状态。"
            )
